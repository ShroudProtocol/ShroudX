...
// A helper object for signing messages from Shroudnodes
...
public:
     ...
     *     1) we have an active Shroudnode,
     *     2) we have a valid Shroudnode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Shroudnode address
    bool CheckSignature(const CPubKey& pubKeyShroudnode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, shroudnode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 50000 SHROUD - checking if valid shroudnode)
...
private:
    ...
    // Keep track of the used Shroudnodes
    std::vector<CTxIn> vecShroudnodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Shroudnode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a Shroudnode to start the process
    ...
    /// Get Shroudnode updates about the progress of mixing
...
public:
    CShroudnode* pSubmittedToShroudnode;
