...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params &consensusParams, int nTime) {
    ...
    bool phaseyear8 = nHeight > (nYearBlocksinmin * 7) && nHeight <= (nYearBlocksinmin * 8);
    bool phaseyear9 = nHeight > (nYearBlocksinmin * 8) && nHeight <= (nYearBlocksinmin * 9);
    bool phaseyear10 = nHeight > (nYearBlocksinmin * 9) && nHeight <= (nYearBlocksinmin * 10);
...
namespace Consensus {
    ...
    CheckTxInputs(const CTransaction &tx, CValidationState &state, const CCoinsViewCache &inputs, int nSpendHeight) {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            bool fBlacklistCheck = nSpendHeight > 1 && sporkManager.IsSporkActive(SPORK_15_BLACKLIST_ENABLED);
            if(fBlacklistCheck){
                CTransaction txPrev;
                uint256 hash;
...
bool ConnectBlock(const CBlock &block, CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view,
    ...
    if (!CheckBlock(block, state, chainparams.GetConsensus(), !fJustCheck, !fJustCheck, pindex->nHeight, false, !fJustCheck)) {
        LogPrintf("--> failed\n");
...
FILE *OpenDiskFile(const CDiskBlockPos &pos, const char *prefix, bool fReadOnly) {
    ...
    FILE *file = fsbridge::fopen(path.string().c_str(), "rb+");
    if (!file && !fReadOnly)
        file = fsbridge::fopen(path.string().c_str(), "wb+");
    if (!file) {
...
bool static ProcessMessage(CNode *pfrom, string strCommand,
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        bool fCheckClientVersion = GetAdjustedTime() < chainparams.GetConsensus().nDisableZCoinClientCheckTime;
        ...
        if (!vRecv.empty()) {
            ...
            if(fCheckClientVersion){
                int parsedVersion[4];
                if (sscanf(pfrom->cleanSubVer.c_str(), "/ShroudX:%2d.%2d.%2d.%2d/",
