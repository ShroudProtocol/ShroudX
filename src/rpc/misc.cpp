...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"shroudaddress\"\n"
            "\nReturn information about the given ShroudX address.\n"
            "\nArguments:\n"
            "1. \"shroudaddress\"     (string, required) The ShroudX address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"shroudaddress\", (string) The ShroudX address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: ShroudX address and we have full public key:
...
UniValue insync(const UniValue& params, bool fHelp)
{
    ...
    if(strMode == "status") {
        ...
        objStatus.push_back(Pair("AssetID", shroudnodeSync.GetAssetID()));
        objStatus.push_back(Pair("AssetName", shroudnodeSync.GetAssetName()));
        objStatus.push_back(Pair("Attempt", shroudnodeSync.GetAttempt()));
        objStatus.push_back(Pair("IsBlockchainSynced", shroudnodeSync.GetBlockchainSynced()));
        objStatus.push_back(Pair("IsShroudnodeListSynced", shroudnodeSync.IsShroudnodeListSynced()));
        objStatus.push_back(Pair("IsWinnersListSynced", shroudnodeSync.IsWinnersListSynced()));
        objStatus.push_back(Pair("IsSynced", shroudnodeSync.IsSynced()));
        objStatus.push_back(Pair("IsFailed", shroudnodeSync.IsFailed()));
        return objStatus;
    }
    ...
    if(strMode == "next")
    {
        ...
        return "sync updated to " + shroudnodeSync.GetAssetName();
    }
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are ShroudX addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) ShroudX address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"shroudaddress\"  (string, required) The ShroudX address to use for the signature.\n"
