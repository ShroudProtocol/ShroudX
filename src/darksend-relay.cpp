...
CDarkSendRelay::CDarkSendRelay(CTxIn& vinShroudnodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinShroudnode = vinShroudnodeIn;
    vchSig = vchSigIn;
...
std::string CDarkSendRelay::ToString()
{
    ...
    info << "vin: " << vinShroudnode.ToString() <<
...
{
    CShroudnode* pmn = mnodeman.GetShroudnodeByRank(nRank, nBlockHeight, MIN_PRIVATESEND_PEER_PROTO_VERSION);
